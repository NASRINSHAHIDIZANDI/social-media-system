...
class Post:
    ...
    def __str__(self):
        post_info = "{} said: {}\nLikes: {}, Comments:".format(self.username, self.content, len(self.likes))
        for comment in self.comments:
            post_info += "\n- {} commented: {}".format(comment['username'], comment['comment'])
        return post_info
        ...
        while True:
            ...
            if choice == '1':
                ...
                if user:
                    ...
                    print "Post created by {} with content: {}".format(post.username, post.content)
                else:
                ...
                user = self.get_user(username)
                if user:
                    post_index = int(raw_input("Enter the post index you want to like: "))
                    author_username = raw_input("Enter the author's username: ")
                ...
                user = self.get_user(username)
                if user:
                    post_index = int(raw_input("Enter the post index you want to comment on: "))
                    author_username = raw_input("Enter the author's username: ")
                    author = self.get_user(author_username)
                    if author:
